...
def search_flights(origin: str, destination: str) -> str:
    ...
    for f in flights:
        ...
        result += f"- {f['airline']} ({f['class']}): Lịch trình {f['departure']} - {f['arrival']} | Giá: {price_str}\n"
...
def search_hotels(city: str, max_price_per_night: int = 99999999) -> str:
    ...
    result = f"Danh sách khách sạn tại {city} (Giá <= {format_currency(max_price_per_night)}/đêm):\n"
    for h in filtered_hotels:
        price_str = format_currency(h['price_per_night'])
        result += f"- {h['name']} ({h['stars']} sao, Khu {h['area']}): {price_str}/đêm | Đánh giá: {h['rating']}/5\n"
...
def calculate_budget(total_budget: int, expenses: str) -> str:
    ...
    expenses: chuỗi mô tả các khoản chi, định dạng 'tên: số tiền, tên: số tiền' (VD: 'vé máy bay: 890000, khách sạn: 650000')
    """
    try:
        expense_dict = {}
        total_expense = 0
        
        # Xử lý chuỗi expenses
        ...
        for name, amount in expense_dict.items():
            result += f"- {name.capitalize()}: {format_currency(amount)}\n"
        ...
        if remaining < 0:
            result += f"\n⚠️ Vượt ngân sách {format_currency(abs(remaining))}! Cần điều chỉnh."
            
        return result
        
    except Exception as e:
        return "Lỗi format chuỗi expenses. Vui lòng đảm bảo định dạng 'tên: số tiền, tên2: số tiền2' (VD: 'vé máy bay: 890000, khách sạn: 650000')."